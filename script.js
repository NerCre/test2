(() => {
  ...
  // Image upload drafts (DataURL) — not exported to CSV
  let entryImageDraft = null;
  let exitImageDraft = null;
  let exitImageDraftForId = null;
  ...
  async function compressImageFileToDataURL(file, opts = {}) {
    const maxDim = Number(opts.maxDim ?? 1400);
    ...
    // Prefer JPEG for size; keep PNG if it was PNG with transparency and user explicitly wants it.
    const outType = "image/jpeg";
    return canvas.toDataURL(outType, clamp(quality, 0.3, 0.95));
  }

  // Entry UI state/render
  function renderEntryImageUI() {
    const wrap = $("#entry-image-preview-wrap");
    const img = $("#entry-image-preview");
    const empty = $("#entry-image-empty");
    const btnOpen = $("#entry-image-open");
    const btnClear = $("#entry-image-clear");

    const has = !!entryImageDraft;
    if (wrap) wrap.hidden = !has;
    if (empty) empty.hidden = has;
    if (img && has) img.src = entryImageDraft;
    if (btnOpen) btnOpen.hidden = !has;
    if (btnClear) btnClear.hidden = !has;
  }

  function clearEntryImageDraft() {
    entryImageDraft = null;
    const f = $("#entry-image-file");
    if (f) f.value = "";
    renderEntryImageUI();
  }

  function initEntryImageUI() {
    const fileEl = $("#entry-image-file");
    const btnClear = $("#entry-image-clear");
    const btnOpen = $("#entry-image-open");
    ...
    if (fileEl) {
      fileEl.addEventListener("change", async () => {
        ...
        if (!file) return;
        try {
          entryImageDraft = await compressImageFileToDataURL(file, { maxDim: 1400, quality: 0.78 });
          renderEntryImageUI();
          showToast("画像をセットしました（エントリー）", "success", 1200);
        } catch (e) {
          console.warn(e);
          showToast("画像の読み込みに失敗しました", "error", 2200);
          fileEl.value = "";
        }
      });
    }

    btnClear?.addEventListener("click", () => clearEntryImageDraft());
    btnOpen?.addEventListener("click", () => {
      if (!entryImageDraft) return;
      window.open(entryImageDraft, "_blank", "noopener");
    });

    renderEntryImageUI();
  }

  // Exit UI state/render
  function renderExitImageUI() {
    const wrap = $("#exit-image-preview-wrap");
    const img = $("#exit-image-preview");
    const empty = $("#exit-image-empty");
    const btnOpen = $("#exit-image-open");
    const btnClear = $("#exit-image-clear");

    const has = !!exitImageDraft;
    if (wrap) wrap.hidden = !has;
    if (empty) empty.hidden = has;
    if (img && has) img.src = exitImageDraft;
    if (btnOpen) btnOpen.hidden = !has;
    if (btnClear) btnClear.hidden = !has;
  }

  function bindExitImageUI(record) {
    // Called every time Exit details are rendered (elements are re-created).
    exitImageDraftForId = record?.id || null;
    exitImageDraft = (record && (record.exitImageData ?? null)) || null;

    const fileEl = $("#exit-image-file");
    const btnClear = $("#exit-image-clear");
    const btnOpen = $("#exit-image-open");
    ...
    if (fileEl) {
      ...
      fileEl.addEventListener("change", async () => {
        ...
        if (!file) return;
        try {
          exitImageDraft = await compressImageFileToDataURL(file, { maxDim: 1400, quality: 0.78 });
          renderExitImageUI();
          showToast("画像をセットしました（決済）", "success", 1200);
        } catch (e) {
          console.warn(e);
          showToast("画像の読み込みに失敗しました", "error", 2200);
          fileEl.value = "";
    ...
    btnOpen?.addEventListener("click", () => {
      if (!exitImageDraft) return;
      window.open(exitImageDraft, "_blank", "noopener");
    });
...
function setDefaultStatsRangeLast7Days() {
  ...
  function saveRecords() {
    try {
      ...
      console.warn("Failed to save records (storage quota?)", e);
      showToast("保存できませんでした（画像が大きい／容量上限の可能性）", "error", 3200);
    }
  }
  ...
  function migrateRecord(r) {
    ...
    // image data is optional but ignored by CSV
    // Legacy: imageData -> entryImageData (for backward compatibility)
...
function getEntryForm() {
  ...
  function clearEntryForm() {
    ...
    // Clear entry image draft UI
    entryImageDraft = null;
    const _ef = $("#entry-image-file");
    if (_ef) _ef.value = "";
    renderEntryImageUI();
...
function onJudge(shouldSave) {
  ...
  const base = {
    ...
    entryImageData: entryImageDraft,
  ...
  updateEntryEditUI();
  $("#entry-form").reset();
  // Clear entry image draft after save
  entryImageDraft = null;
  const _ef2 = $("#entry-image-file");
  if (_ef2) _ef2.value = "";
  renderEntryImageUI();
  ...
  // Exit image UI (per selected trade)
  bindExitImageUI(r);
      return;
    }
...
`;
  ...
  <div class="image-upload-card">
    ...
    <div class="image-upload-row">
      <input type="file" id="exit-image-file" accept="image/*">
      <button type="button" id="exit-image-clear" class="btn-ghost btn-small">画像を削除</button>
      <button type="button" id="exit-image-open" class="btn-primary btn-small">別タブで開く</button>
    </div>

    <div id="exit-image-preview-wrap" class="image-preview-wrap" hidden>
      <img id="exit-image-preview" alt="決済画像プレビュー">
    </div>
    <p id="exit-image-empty" class="muted small">未設定</p>
    <p class="muted small">※ 画像はJSONに含まれます（CSVには出力されません）。</p>
  </div>
...
`;
  ...
  function updateExitProfitPreview(record) {
    ...
    record._tmp = { exitPrice, highDuringTrade: high, lowDuringTrade: low, postExitMarket, taeValue, exitReason, exitReasonOther, resultMemo: memo, profit };
  }
  ...
  function clearExitFormOnly() {
    ...
    // Restore image draft to the saved value (if any)
    exitImageDraftForId = r.id;
    exitImageDraft = r.exitImageData ?? null;
    const _xf = $("#exit-image-file");
    if (_xf) _xf.value = "";
    renderExitImageUI();
    const disp = $("#exitProfitDisp");
    if (disp) {
    ...
    r._tmp = { exitPrice: null, highDuringTrade: null, lowDuringTrade: null, postExitMarket: "", taeValue: null, exitReason: "", exitReasonOther: "", resultMemo: "", profit: null };
  ...
  function saveExit() {
    ...
    const updated = migrateRecord({
      ...
      exitReasonOther,
      resultMemo: memo,
      exitImageData: (exitImageDraftForId === selectedExitId ? exitImageDraft : (r.exitImageData ?? null)),
      profit,
...
function populateIndicatorStateFilterOptions(indicatorKey) {
  ...
  function loadRecordToEntry(id) {
    ...
    // Load entry image (if any)
    entryImageDraft = r.entryImageData ?? r.imageData ?? null;
    const _ef = $("#entry-image-file");
    if (_ef) _ef.value = "";
    renderEntryImageUI();
  ...
  function bind() {
    ...
    elExitSelect?.addEventListener("change", () => {
      ...
      if (!selectedExitId) {
        ...
        if (elExitDetailsSide) elExitDetailsSide.innerHTML = ``;
        updateExitEditUI();
        // Clear exit image draft (no selection)
        exitImageDraft = null;
        exitImageDraftForId = null;
        return;
      }
