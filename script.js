...
function initTabs() {
  ...
  // Statsの「フィルタリングしたいインジ」用（決め手インジと同じマスターを使う）
  // 万一DECISIVE_INDICATORSが読み込めない/空の場合でも落ちないようにフォールバックを用意する
  const INDICATOR_FILTER_ITEMS = (Array.isArray(DECISIVE_INDICATORS) && DECISIVE_INDICATORS.length)
    ? DECISIVE_INDICATORS
    : [
      { value: "waveCount", label: "波動ライン" },
      { value: "dowShape", label: "ダウ形状" },
  ...
  function populateDecisiveIndicatorSelects() {
    ...
    const elDyn = document.getElementById("filter-indicatorKey");

    const fill = (el, firstText, items) => {
      if (!el) return;
    ...
    fill(elEntry, "（未選択）", DECISIVE_INDICATORS);
    fill(elFilter, "（全て）", DECISIVE_INDICATORS);
    // ここが今回追加分：フィルタ用はフォールバック込みのマスターを使う
    fill(elDyn, "（未選択）", INDICATOR_FILTER_ITEMS);
  }
      // restore if possible
      el.value = current;
    };
...
function onJudge(shouldSave) {
  ...
  <label>決済日時
    <input id="exit-datetime" type="datetime-local" value="${r.datetimeExit ?? ""}">
  </label>
...
`;
  ...
  function updateExitProfitPreview(record) {
    ...
    record._tmp = { exitPrice, highDuringTrade: high, lowDuringTrade: low, exitReason, exitReasonOther, resultMemo: memo, profit };
  }
  ...
  function clearExitFormOnly() {
    ...
    r._tmp = { exitPrice: null, highDuringTrade: null, lowDuringTrade: null, exitReason: "", exitReasonOther: "", resultMemo: "", profit: null };
  ...
  function getFilters() {
    return {
      ...
      indicatorValue: $("#filter-indicatorValue")?.value || "",
  ...
  function applyFilters(list, f) {
    return list.filter((r) => {
      ...
      // Dynamic indicator filter (two-step)
      if (f.indicatorKey && f.indicatorValue) {
        const m = DECISIVE_FIELD_MAP[f.indicatorKey];
        if (!m) return false;
        const prop = m.prop;
        if ((r[prop] || "") !== f.indicatorValue) return false;
      }
...
function clearDateRangeFilter() {
  ...
  // Dynamic indicator filter (Stats): choose an indicator, then choose its state
  function populateIndicatorValueFilterOptions(indicatorKey) {
    const elVal = document.getElementById("filter-indicatorValue");
    if (!elVal) return;

    const current = elVal.value || "";
    elVal.innerHTML = "";

    // Always have "all" option first
    const opt0 = document.createElement("option");
    opt0.value = "";
    opt0.textContent = "（全て）";
    elVal.appendChild(opt0);
    ...
    const src = document.getElementById(m.elId);
    ...
    for (const opt of Array.from(src.options)) {
      ...
      if (!v) continue; // skip placeholder
      const o = document.createElement("option");
      o.value = v;
      o.textContent = String(opt.textContent || v).trim();
      elVal.appendChild(o);
    }

    elVal.disabled = false;
    // restore if possible (only if option exists)
    if ([...elVal.options].some((o) => o.value === current)) elVal.value = current;
    else elVal.value = "";
  }
...
function safeRenderStats() {
  ...
  function bind() {
    ...
    $("#filter-decisiveIndicator")?.addEventListener("change", () => {
      ...
      const candidates = applyFilters(records, fNoSig);
      populateDecisiveSignalFilterOptions(candidates, f.decisiveIndicator);
    // Dynamic indicator filter (Stats): rebuild "state" candidates when indicator changes
    $("#filter-indicatorKey")?.addEventListener("change", () => {
      const elVal = $("#filter-indicatorValue");
      if (elVal) elVal.value = "";
      populateIndicatorValueFilterOptions($("#filter-indicatorKey")?.value || "");
    });
    ...
    $("#btn-clear-filter")?.addEventListener("click", () => {
      ...
      $("#filter-indicatorValue").value = "";
      populateIndicatorValueFilterOptions("");
      $("#filter-result").value = "";
      $("#filter-start").value = "";
      $("#filter-end").value = "";
      safeRenderStats();
...
function init() {
  ...
  // Debug: フィルタ用インジが空のときは、script.jsが差し替わってない or JSエラーの可能性
  try {
    const el = document.getElementById('filter-indicatorKey');
    if (el && el.options && el.options.length <= 1) {
      console.warn('[EdgeScope] filter-indicatorKey has no options. Check that index.html loads the updated script.js.');
    }
  } catch(e) {}
