...
function initTabs() {
  ...
  // NEW: Statsの「フィルタリングしたいインジ」（動的インジフィルタ）
  ...
  const fill = (el, firstText, items) => {
    ...
    // restore if possible
    el.value = Array.from(el.options).some((o) => o.value === current) ? current : "";
  };

  fill(elEntry, "（未選択）", DECISIVE_INDICATORS);
  fill(elFilter, "（全て）", DECISIVE_INDICATORS);

  // 「フィルタリングしたいインジ」も同じマスターから生成（一覧に全インジを出す必要はない）
  fill(elDyn, "（未選択）", DECISIVE_INDICATORS);
}
...
if (f.indicatorKey && f.indicatorValue) {
  ...
  const prop = m?.prop || f.indicatorKey;
...
// NEW: Statsの動的インジフィルタ（インジを選んだら、そのインジの「状態」候補を自動生成）
function populateIndicatorValueFilterOptions(indicatorKey) {
  ...
  elVal.value = Array.from(elVal.options).some((o) => o.value === current) ? current : "";
}
...
// NEW: 動的インジフィルタ（Stats）: インジを選んだら「状態」候補を組み替える
$("#filter-indicatorKey")?.addEventListener("change", () => {
  ...
  populateIndicatorValueFilterOptions($("#filter-indicatorKey")?.value || "");
    ...
    $("#btn-clear-filter")?.addEventListener("click", () => {
      ...
      if ($("#filter-indicatorKey")) $("#filter-indicatorKey").value = "";
      if ($("#filter-indicatorValue")) $("#filter-indicatorValue").value = "";
