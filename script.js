...
function parseISODateOnly(value) {
  ...
  function weekdayLabelFromDatetimeLocal(value) {
    const dt = parseDateTimeLocalToDate(value);
    if (!dt) return "";
    return WEEKDAY_JP[dt.getDay()] || "";
  }

  function tradeDurationText(datetimeEntry, datetimeExit) {
    const a = parseDateTimeLocalToDate(datetimeEntry);
    const b = parseDateTimeLocalToDate(datetimeExit);
    if (!a || !b) return "";
  ...
  function updateEntryWeekdayFromInput() {
    const elW = $("#entry-weekday");
    const elDt = $("#entry-datetime");
    if (!elW || !elDt) return;
    elW.value = weekdayLabelFromDatetimeLocal(elDt.value);
  }
...
function setDefaultStatsRangeLast7Days() {
  ...
  function migrateRecord(r) {
    ...
    out.entryWeekday = out.entryWeekday || weekdayLabelFromDatetimeLocal(out.datetimeEntry);
...
function getEntryForm() {
    return {
      ...
      entryWeekday: weekdayLabelFromDatetimeLocal($("#entry-datetime").value || null),
  ...
  function clearEntryForm() {
    editingEntryId = null;
    $("#entry-form").reset();
  if ($("#entry-session")) $("#entry-session").value = "";
  if ($("#entry-tradeMethod")) $("#entry-tradeMethod").value = "";
  if ($("#entry-weekday")) $("#entry-weekday").value = "";
  updateEntryWeekdayFromInput();
...
function onJudge(shouldSave) {
  ...
  function updateExitSelect() {
    ...
    const dtExit = $("#exit-datetime")?.value || record.datetimeExit || null;
    const durEl = $("#exitDurationDisp");
    if (durEl) {
      const dur = tradeDurationText(record.datetimeEntry, dtExit);
...
function populateIndicatorStateFilterOptions(indicatorKey) {
  ...
  function loadRecordToEntry(id) {
    ...
    $("#entry-weekday") && ($("#entry-weekday").value = r.entryWeekday || weekdayLabelFromDatetimeLocal(r.datetimeEntry));
    $("#entry-session") && ($("#entry-session").value = r.entrySession || "");
    $("#entry-tradeMethod") && ($("#entry-tradeMethod").value = r.tradeMethod || "");
...
function safeRenderStats() {
  ...
  function exportCSV() {
    ...
    const cols = [
      ...
      "datetimeEntry","entryWeekday","entrySession","tradeMethod","symbol","timeframe","tradeType","directionPlanned",
      ...
      "hasResult","datetimeExit","tradeDuration","exitPrice","highDuringTrade","lowDuringTrade","exitReason","exitReasonOther","profit",
    ...
    for (const r0 of records) {
      ...
      const row = cols.map((c) => {
        if (c === "tradeDuration") return esc(tradeDurationText(r.datetimeEntry, r.datetimeExit));
        if (c === "entryWeekday") return esc(r.entryWeekday || weekdayLabelFromDatetimeLocal(r.datetimeEntry));
        return esc(r[c]);
      });
      lines.push(row.join(","));
    }
  ...
  function bind() {
    ...
    $("#entry-datetime")?.addEventListener("input", updateEntryWeekdayFromInput);
    $("#entry-datetime")?.addEventListener("change", updateEntryWeekdayFromInput);
