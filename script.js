(() => {
  ...
  // Open stored image in a new tab reliably (works well even for large data URLs)
  function openImageInNewTab(dataUrl, title = "image") {
    if (!dataUrl) return;
...
</style></head><body><img src="${dataUrl}" alt="${t}"></body></html>`);
    w.document.close();
  }
...
`;
    ...
    $("#exit-image-open")?.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      openImageInNewTab(exitImageData, "決済画像");
    });
...
function safeRenderStats() {
  ...
  function bind() {
    ...
    $("#entry-image-open")?.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      openImageInNewTab(entryImageData, "エントリー画像");
    });
